    ...
    while len(ban)+n_step<len(features)-k:
        ...
        for i in range(0,len(features)):
            if features[i] in ban:
                ...
                if i!=0:
                    metric.append(np.mean(cross_val_score(model, traintestX.drop(ban[1:]+[features[i]],axis=1), traintesty, scoring=control.SelectionParameters['Metric'].GetValue(), cv=cv, n_jobs=-1, error_score='raise')))
                else:
                    metric.append(np.mean(cross_val_score(model, traintestX.drop(ban[1:],axis=1), traintesty, scoring=control.SelectionParameters['Metric'].GetValue(), cv=cv, n_jobs=-1, error_score='raise')))
        ...
        for i in range(0,len(features)):
            if features[i] in chosen:
                ...
                if i==0 and len(chosen)==1:
                    ...
                    if i!=0:
                        metric.append(np.mean(cross_val_score(model, traintestX[chosen[1:]+[features[i]]], traintesty, scoring=control.SelectionParameters['Metric'].GetValue(), cv=cv, n_jobs=-1, error_score='raise')))
                    else:
                        metric.append(np.mean(cross_val_score(model, traintestX[chosen[1:]], traintesty, scoring=control.SelectionParameters['Metric'].GetValue(), cv=cv, n_jobs=-1, error_score='raise')))
