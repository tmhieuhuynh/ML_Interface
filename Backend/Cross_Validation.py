...
def evaluate_model(control, model, X, y):
	...
	scores1 = cross_val_score(model, X, y, scoring='neg_mean_absolute_percentage_error', cv=cv, n_jobs=None, error_score='raise')
	scores2 = cross_val_score(model, X, y, scoring='neg_root_mean_squared_error', cv=cv, n_jobs=None, error_score='raise')
	scores3 = cross_val_score(model, X, y, scoring='r2', cv=cv, n_jobs=None, error_score='raise')
